...
use crate::api::configuration::{Settings, get_files_configuration};
use crate::api::application::Application;
use crate::connectors::FilesConnectorType;
use crate::connectors::inner::{InnerFilesConnector, inner_middleware_fn};
use crate::connectors::web::{WebFilesConnector, web_middleware_fn};
...
pub async fn spawn_configuration() -> Settings {
    ...
    let configuration = {
        let mut c = get_files_configuration(
            Some(
...
pub async fn spawn_connector_app(
    ...
    connector_type: FilesConnectorType,
) -> ConnectorTestApp {
    tracing();
            ...
            HttpServer::new(move || {
                App::new()
                    ...
                    .service(
                        ...
                        .connector_service(Some("/template"), connector_server.get_ref())
...
async fn inner_health_check(connector: Data<InnerFilesConnector>) -> HttpResponse {
...
async fn web_health_check(connector: Data<WebFilesConnector>) -> HttpResponse {
    match connector.health_check().await {
        Ok(_) => build_http_response_empty_data(),
        Err(e) => build_http_response_error_data(e),
    }
