...
use crate::api::configuration::{Settings, get_template_configuration};
use crate::api::application::Application;
use crate::connectors::TemplateConnectorType;
use crate::connectors::inner::{InnerTemplateConnector, inner_middleware_fn};
use crate::connectors::web::{WebTemplateConnector, web_middleware_fn};
...
pub async fn spawn_configuration() -> Settings {
    ...
    let configuration = {
        let mut c = get_template_configuration(
            Some(
...
pub async fn spawn_connector_app(
    ...
    connector_type: TemplateConnectorType,
) -> ConnectorTestApp {
    tracing();
...
async fn inner_health_check(connector: Data<InnerTemplateConnector>) -> HttpResponse {
...
async fn web_health_check(connector: Data<WebTemplateConnector>) -> HttpResponse {
    match connector.health_check().await {
        Ok(_) => build_http_response_empty_data(),
        Err(e) => build_http_response_error_data(e),
    }
